...
bool CollisionManager::PlayerCollision(const SDL_Rect player, const int dX, const int dY)
{
	...
	Tile* tiles[4] = { G.GetLevel()[playerY][playerX],																				// Player's tile.
					   G.GetLevel()[playerY][(playerX + 1 == COLS ? COLS-1 : playerX + 1)],										// Right tile.
					   G.GetLevel()[(playerY + 1 == ROWS ? ROWS-1 : playerY + 1)][(playerX + 1 == COLS ? COLS-1 : playerX + 1)],	// Bottom-Right tile.
					   G.GetLevel()[(playerY + 1 == ROWS ? ROWS-1 : playerY + 1)][playerX] };										// Bottom tile.
	for (int i = 0; i < 4; i++)
	{
